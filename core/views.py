...
class homescreen(viewsets.ViewSet):  # Homescreen class
    ...
    def homescreen_filters(self, request):
        ...
        if user_obj != '':
            ...
            # filter_request = UserFilterData.objects.filter(user_id=user_obj.id)
            #
            # if filter_request:
            #     filter_request.update(interested_age=request_data.get('interested_age'),
            #                           max_interested_age=request_data.get('max_interested_age'),
            #                           kids=request_data.get('kids'), drink=request_data.get('drink'),
            #                           smoke=request_data.get('smoke'),
            #                           relegion=request_data.get('relegion'), city=request_data.get('city'),
            #                           height=request_data.get('height'), country=request_data.get('country'),
            #                           exercise=request_data.get('exercise'),
            #                           education=request_data.get('education'))
            # else:
            #     UserFilterData.objects. \
            #         create(user_id=user_obj.id,
            #                interested_age=request_data.get('interested_age'),
            #                max_interested_age=request_data.get('max_interested_age'),
            #                kids=request_data.get('kids'), drink=request_data.get('drink'),
            #                smoke=request_data.get('smoke'),
            #                relegion=request_data.get('relegion'), city=request_data.get('city'),
...
class UserFilterViewSet(viewsets.ModelViewSet):
    from core.serializers import UserFilteredSerializer
    serializer_class = UserFilteredSerializer
    permission_classes = (IsAuthenticated,)

    def list(self, request, *args, **kwargs):
        requested_user = User_Profile.objects.get(user_id=self.request.user)
        check = UserFilterData.objects.filter(user_id=requested_user)
        if check:
            queryset = UserFilterData.objects.filter(user_id=requested_user)
    ...
    def filter_reset(self, request):
        requested_user = User_Profile.objects.get(user_id=self.request.user)
        delete_check = UserFilterData.objects.filter(user_id=requested_user).delete()
        return Response({'message': "Filter reset"}, status=status.HTTP_200_OK)
